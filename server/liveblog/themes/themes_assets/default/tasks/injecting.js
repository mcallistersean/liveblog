...
const indexInject = (
  ...
  return () => {
    ...
    if (
      apiResponse.posts._items.length > 0 &&
      (protocol && apiHost && blogId)
    ) {
    ...
    var indexTask = gulp
      .src(fs.existsSync(index) ? index : resolveResource(index, theme, CWD))
      .pipe(plugins.inject(sources))
      .pipe(
        plugins.nunjucks.compile(
          {
    ...
    if (theme.ampTheme) {
      indexTask = indexTask.pipe(
        plugins.inject(lessCommon(false, theme, inputPath), {
...
const templateInject = (theme, inputPath) => {
  return () => {
    ...
    templates.push(
      fs.existsSync(timeline) ? timeline : resolveResource(timeline, theme, CWD)
    );
    ...
    return (
      gulp
        ...
        .pipe(
          plugins.inject(gulp.src(templates), {
...
const themeReplace = (theme, paths, callback) => {
  return () => {
    ...
    gulp
      .src("theme.json", { base: base })
      .pipe(
        plugins.replace(
          cssName,
          manifest[paths.cssfile] || manifest[`${theme.name}.css`]
        )
      )
      .pipe(
        plugins.replace(
          jsName,
          manifest[paths.jsfile] || manifest[`${theme.name}.js`]
        )
      )
      .pipe(
        plugins.replace(
