...
const {
  discoverThemePaths,
  themeDependencyPath
} = require("liveblog-shared-tools").utils;
...
const lessCommon = (cleanCss, theme, inputPath) => {
  ...
  if (!theme.onlyOwnCss && theme.extends) {
    ...
    lessFiles.push(
      fs.existsSync(themeLess)
        ? themeLess
        : path.resolve(`${inputPath}/less/*.less`)
    );
  }
...
const wireDeps = theme => {
  ...
  const subAction = theme => {
    if (theme.extends) {
      ...
      if (theme.extendsFromPackage) packageName = theme.extendsFromPackage;

      exists(packageName).then(found => {
        if (!found) {
          console.error(
            `Theme parent dependency not found in npm registry ${packageName}`
          );
          return;
        }
        ...
        const parentTheme = require(path.resolve(
          parentThemePath,
          "theme.json"
        ));
