...
if (argvKey !== 0) {
  match = process.argv[argvKey].match(
    /^(http:\/\/|https:\/\/|\/\/)([^\/]+)\/(api\/client_blogs|embed|[^\/]+\/blogs)\/(\w+)/i
  );
}
...
if (match.length > 0) {
  ...
  request.get(
    `${postsEndpoint}?max_results=${postsPerPage}&source=${JSON.stringify(
      query
    )}`,
    response => {
      let body = "";
...
const ampSupport = item => {
  ...
  if (item["groups"] && item["groups"][1]["refs"]) {
    ...
    if (notSupported.length > 0) return false;
  }
...
gulp.task(
  "precomp-parent-templates",
  precompileParentTemplates(theme, nunjucksEnv)
);

gulp.task(
  "precomp-theme-templates",
  precompileThemeTemplates(theme, nunjucksEnv)
);
...
gulp.task(
  "template-inject",
  ["less", "bundlejs"],
  templateInject(theme, inputPath)
);
...
gulp.task(
  "theme-replace",
  ["bundlejs", "less"],
  themeReplace(theme, paths, loadThemeJSON)
);
